...
function startInspectionOnTab(tabId) {
  ...
  chrome.tabs.get(tabId, (tab) => {
    if (chrome.runtime.lastError) {
      console.error('MDS Style Inspector: Tab not found or inaccessible:', tabId, chrome.runtime.lastError.message);
      return;
    }
    
    if (!tab.url || !tab.url.startsWith('http')) {
      console.log('MDS Style Inspector: Tab is not a web page, skipping:', tabId, tab.url);
      return;
    }
    
    console.log('MDS Style Inspector: Tab is valid, attempting to start inspection:', tabId, tab.url);
    ...
    setTimeout(() => {
      chrome.tabs.sendMessage(tabId, { action: 'startInspection' }, (response) => {
        if (chrome.runtime.lastError) {
          console.log('MDS Style Inspector: Content script not ready, retrying in 3 seconds...', chrome.runtime.lastError.message);
          // Retry after a longer delay
          setTimeout(() => {
            chrome.tabs.sendMessage(tabId, { action: 'startInspection' }, (retryResponse) => {
              if (chrome.runtime.lastError) {
                console.error('MDS Style Inspector: Failed to start inspection after retry for tab:', tabId, chrome.runtime.lastError.message);
                ...
                console.error('MDS Style Inspector: Content script responded but failed to start inspection on retry for tab:', tabId);
              }
            });
          }, 3000);
          ...
          console.error('MDS Style Inspector: Content script responded but failed to start inspection for tab:', tabId, response);
        }
      });
...
function stopInspectionOnTab(tabId) {
  ...
  chrome.tabs.sendMessage(tabId, { action: 'stopInspection' }, (response) => {
    if (chrome.runtime.lastError) {
      ...
      console.log('MDS Style Inspector: Content script not available for tab:', tabId, chrome.runtime.lastError.message);
    } else if (response && response.success) {
      console.log('MDS Style Inspector: Inspection stopped successfully for tab:', tabId);
    } else {
      console.log('MDS Style Inspector: Content script responded but stop failed for tab:', tabId);
    }
  });
}
    ...
    allTabs.forEach(tab => {
      if (tab.url && tab.url.startsWith('http')) {
        chrome.tabs.sendMessage(tab.id, { 
          ...
          if (chrome.runtime.lastError) {
            // This is expected when content script is not available
            console.log('MDS Style Inspector: Content script not available for tab:', tab.id, chrome.runtime.lastError.message);
          }
        });
      }
    });
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (changeInfo.status === 'complete' && tab.url && tab.url.startsWith('http')) {
    ...
    if (globalAutoInspectionEnabled) {
      ...
      console.log('MDS Style Inspector: Auto-inspection disabled, not starting for tab:', tabId);
    }
  }
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'contentScriptReady') {
    ...
    console.log('MDS Style Inspector: Content script ready notification received from tab:', sender.tab?.id);
