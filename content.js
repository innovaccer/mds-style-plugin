  ...
  class StyleInspector {
    constructor() {
      ...
      this.hardcodedPatterns = {
        ...
        colors:
          /(#[0-9a-fA-F]{3,6}|rgb\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*\)|rgba\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*,\s*[\d.]+\s*\)|#[0-9a-fA-F]{8}|hsl\(\s*\d+\s*,\s*\d+%\s*,\s*\d+%\s*\)|hsla\(\s*\d+\s*,\s*\d+%\s*,\s*\d+%\s*,\s*[\d.]+\s*\))/,

        // Font sizes
        fontSize:
          /(\d+(?:\.\d+)?)\s*(px|rem|em|pt|pc|in|cm|mm|vh|vw|vmin|vmax)/,
    ...
    init() {
      ...
      chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
        console.log(
          'MDS Style Inspector: Content script received message:',
          request.action
        );
        ...
        try {
          switch (request.action) {
            case 'startInspection':
              console.log(
                'MDS Style Inspector: Starting inspection from message'
              );
            ...
            case 'stopInspection':
              console.log(
                'MDS Style Inspector: Stopping inspection from message'
              );
              this.stopInspection();
              sendResponse({ success: true, stats: this.stats });
              break;
            case 'getStatus':
              console.log('MDS Style Inspector: Returning status:', {
            ...
            case 'updateAutoInspection':
              console.log(
                'MDS Style Inspector: Updating auto-inspection setting:',
                request.isEnabled
              );
            ...
            default:
              console.log(
                'MDS Style Inspector: Unknown message action:',
                request.action
              );
      ...
      console.log(
        'Step 1: Style Inspector: Content script initialized and ready'
      );
      ...
      setTimeout(() => {
        try {
          chrome.runtime.sendMessage(
            ...
            (response) => {
              if (chrome.runtime.lastError) {
                console.log(
                  'MDS Style Inspector: Could not notify background script of readiness:',
                  chrome.runtime.lastError.message
                );
              } else {
                console.log(
                  'MDS Style Inspector: Background script notified of content script readiness'
                );
              }
            }
          );
          ...
          if (
            ...
            console.log(
              'MDS Style Inspector: Extension context invalidated, skipping ready notification'
            );
          } else {
            console.error(
              'MDS Style Inspector: Error sending ready notification:',
              error
            );
          }
        }
    ...
    getAutoInspectionStatusAndStart() {
      console.log(
        'MDS Style Inspector: Getting auto-inspection status from background...'
      );
        ...
        chrome.runtime.sendMessage(
          ...
          (response) => {
            if (chrome.runtime.lastError) {
              ...
              console.log(
                'MDS Style Inspector: Auto-inspection setting from background:',
    ...
    testHighlight() {
      ...
      console.log(
        `Test: Found ${this.stats.hardcodedCount} elements with hardcoded values`
      );
    ...
    startInspection() {
      console.log('Step 3: Style Inspector: Starting inspection...', {
        isActive: this.isActive,
      ...
      if (this.isActive) {
        console.log(
          'MDS Style Inspector: Resetting state for new page inspection'
        );
        this.clearHighlights();
        this.stats.hardcodedCount = 0;
        this.stats.inspectedCount = 0;
    ...
    shouldInspectElement(element) {
      ...
      if (
        element.id &&
        this.reactElementPatterns.some((pattern) => pattern.test(element.id))
      ) {
        return false;
      }
    ...
    inspectPage() {
      ...
      allElements.forEach((element, index) => {
        ...
        if (shouldInspect) {
          inspectedCount++;
          console.log(
            `${inspectedCount}. INSPECTING: ${element.tagName}${element.className ? ' (class: ' + element.className + ')' : ''}${element.id ? ' (id: ' + element.id + ')' : ''}`
          );
        } else {
          skippedCount++;
          console.log(
            `SKIPPED: ${element.tagName}${element.className ? ' (class: ' + element.className + ')' : ''}${element.id ? ' (id: ' + element.id + ')' : ''}`
          );
        }
      });

      console.log(
        `Will inspect ${inspectedCount} stylable elements, skip ${skippedCount} non-stylable elements`
      );

      // Process elements in batches to prevent blocking
      const elementsToInspect = Array.from(allElements).filter((element) =>
        this.shouldInspectElement(element)
      );
    ...
    inspectElement(element) {
      ...
      const hasHardcodedValues = this.checkForHardcodedValues(
    ...
    checkForHardcodedValues(computedStyle, element) {
      ...
      const matchedRules = this.analyzeHardcodedRules(
        element,
        relevantProperties
      );
      ...
      if (matchedRules.length > 0) {
        console.log(
          `Found ${matchedRules.length} hardcoded rules for element:`,
          element.tagName,
    ...
    isHardcodedValue(value, property, element) {
      ...
      const hasTokens = this.tokenPatterns.some((pattern) =>
        pattern.test(value)
      );
      if (hasTokens) {
    ...
    highlightElement(element) {
      if (this.highlightedElements.has(element)) {
        console.log(
          'Element already highlighted, skipping:',
          element.tagName,
          element.className
        );
      ...
      if (element.hasAttribute('data-mds-hardcoded')) {
        console.log(
          'Element already has hardcoded attribute, skipping:',
          element.tagName,
          element.className
        );
        return;
      ...
      this.tooltip.attachTooltip(
        element,
        this.hardcodedProperties.get(element)
      );
      ...
      console.log(
        'Element data attribute after highlighting:',
        element.getAttribute('data-mds-hardcoded')
      );
      console.log(
      ...
      console.log(
        'Element computed style outline:',
        window.getComputedStyle(element).outline
      );

      // Verify the highlight is applied
      const hasHighlightClass = element.classList.contains(
        'mds-style-inspector-highlight'
      );
    ...
    updateStats() {
      ...
      try {
        ...
        if (
          ...
          console.log(
            'MDS Style Inspector: Extension context invalidated, skipping stats update'
          );
        } else {
